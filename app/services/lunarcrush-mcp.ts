...
class LunarCrushMCPService {
  ...
  async initializeConnection(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.sseConnection = https.request(options, (res) => {
        ...
        console.log(`✅ MCP Connected - Session: ${this.sessionId?.substring(0, 10)}...`);
        ...
        res.on('data', chunk => {
          ...
          for (let i = 0; i < lines.length - 1; i++) {
            ...
            else if (line.startsWith('data: {') && line.includes('"jsonrpc"')) {
              try {
      ...
      this.sseConnection.setTimeout(30000, () => reject(new Error('Connection timeout')));
  ...
  private async makeRequest(method: string, params?: any): Promise<any> {
    if (!this.messageEndpoint) {
      throw new Error('MCP not initialized. Call initializeConnection() first.');
    }
      ...
      const req = https.request(options, (res) => {
        if (res.statusCode !== 202 && res.statusCode !== 200) {
          ...
          res.on('end', () => {
            ...
            reject(new Error(`Request failed: ${res.statusCode} - ${errorData}`));
          });
        }
      });
