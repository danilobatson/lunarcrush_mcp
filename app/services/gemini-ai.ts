...
class GeminiAIService {
  ...
  private createAnalysisPrompt(data: CryptoData): string {
    return `
You are an expert cryptocurrency analyst. Analyze the following data for ${data.symbol} and provide a trading recommendation.
...
`;
   ...
   */
  private parseGeminiResponse(response: GeminiResponse, cryptoData: CryptoData): TradingSignal {
    try {
      ...
      return {
        ...
        social_sentiment: analysis.social_sentiment as 'bullish' | 'bearish' | 'neutral',
        data: cryptoData
      };
    } catch (error) {
      ...
      return {
        ...
        recommendation: 'HOLD',
        confidence: 50,
        reasoning: 'Unable to complete full AI analysis. Recommending HOLD for safety.',
