...
export default function TradingIndex() {
	...
	// For now, let's bypass the MCP connection that's causing issues and use direct analysis
	// This will be re-enabled once the url module issue is resolved
	const mcpEnabled = true; // Set to true when MCP is working
	...
	const {
		...
		url: mcpEnabled
			? `https://lunarcrush.ai/sse?key=${env.LUNARCRUSH_API_KEY}`
			: '',
		clientName: 'LunarCrush MCP SSE',
		autoReconnect: mcpEnabled,
		timeout: 15000, // 15 second timeout
	});

	// Override state when MCP is disabled
	const actualState = mcpEnabled ? state : 'failed';
	const actualTools = mcpEnabled ? tools : [];

	console.log('MCP enabled:', mcpEnabled);
	console.log('useMcp tools:', actualTools);
	console.log('useMcp state:', actualState);

	// Add a timeout effect to handle stuck connections
	useEffect(() => {
		const timer = setTimeout(() => {
			if (actualState === 'discovering' || actualState === 'connecting') {
